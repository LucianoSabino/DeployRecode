...
module.exports = {
  ...
  async gerarPdfRelatorioPortoId(req, res) {
    if (req.body?.id !== undefined) {
      ...
      const usuariosConcluiramPortoSucesso = await relatoriosPortosId(
        portoId
      ).catch((err) => {
        console.error(err);
        res.status(500).json("Erro Interno do Servidor");
      ...
      const porto = await prisma.porto
        .findUnique({
          where: {
            id: portoId,
          },
