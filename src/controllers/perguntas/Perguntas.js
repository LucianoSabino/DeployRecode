...
module.exports = {
        ...
        if (acessaPort !== undefined && acessaPort !== null) {
          await prisma.pergunta
            ...
            .then((perguntas) => {
              ...
              let perguntaAtual = perguntas.find(
                (pergunta) => pergunta.id === acessaPort.perguntaAtualId
              );

              // Filtrar as perguntas a partir da perguntaAtualId
              let perguntasSeguintes = perguntas.filter(
                (pergunta) => pergunta.id > acessaPort.perguntaAtualId
              );

              // Reorganizar o array para colocar a perguntaAtualId no começo
              let perguntasParaFrontend = [
                perguntaAtual,
                ...perguntasSeguintes,
              ];
  ...
  async respondendoPerguntas(req, res) {
    let userId = req.user.id;
    if (
      req.body.perguntaId !== undefined &&
      req.body.alternativaMarcadaId !== undefined
    ) {
      let { perguntaId, alternativaMarcadaId } = req.body;

      let acessaPorto = await prisma.acessaPorto
        .findFirst({
          where: {
            userId: userId,
            estadoPortoId: 1,
      ...
      if (acessaPorto !== null || acessaPorto?.length > 0) {
        ...
        let ultimaPerguntaDoPorto =
          perguntasDoPorto[perguntasDoPorto.length - 1];

        // Verificar se a pergunta atual do usuário é a última pergunta do porto
        let estaNaUltimaPergunta =
          perguntaAtual.id === ultimaPerguntaDoPorto.id;
        ...
        if (perguntaId == perguntaAtual.id) {
          ...
          let alternativas = porto.perguntas.find(
          ...
          if (
            perguntaAtual.multiplasAlternativas === false &&
            alternativaMarcadaId.length > 1
          ) {
            ...
            if (estaNaUltimaPergunta) {
              ...
              let acessaPortoFinal = await prisma.acessaPorto
                .findFirst({
                  where: {
                    userId: userId,
                    estadoPortoId: 1,
              ...
              let perguntasCertas = verificarRespostas?.length
                ? verificarRespostas.length
                : 0;
              let perguntasErradas = perguntas
                ? perguntas.length - perguntasCertas
                : 0;
              let resultado = 0;
              let mensagem;
              Promise.all(verificarRespostas).then(async () => {
                let porcentagem =
                  (perguntasCertas / (perguntasCertas + perguntasErradas)) *
                  100;
                if (porcentagem >= 50) {
                  resultado = 2;
                  mensagem = "Passou";
